...
// fetch store products by advanced filters api for most fav
router.get('/list/fav', async (req, res) => {
  try {
    let {
      sortOrder,
      rating,
...
router.get('/list/rated', async (req, res) => {
  try {
    let {
      sortOrder,
      rating,
  ...
  async (req, res) => {
    try {
      ...
      if (req.user.merchant) {
        ...
        products = await Product.find({ user: req.user._id }).populate({
          path: 'brand',
          populate: {
            path: 'merchant',
            model: 'Merchant'
