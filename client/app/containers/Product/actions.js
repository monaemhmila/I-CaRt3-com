...
// fetch/filter store products api fav
export const filterProductsfav = (n, v) => {
  return async (dispatch, getState) => {
    try {
      dispatch(setProductLoading(true));
      const advancedFilters = getState().product.advancedFilters;
      ...
      dispatch({ type: SET_ADVANCED_FILTERS, payload });
      const sortOrder = getSortOrder(payload.order);
      const response = await axios.get(`${API_URL}/product/list/fav`, {
        params: { ...payload, sortOrder }
      });
      const { products, totalPages, currentPage, count } = response.data;
...
// fetch/filter store products api fav
export const filterProductsRated = (n, v) => {
  return async (dispatch, getState) => {
    try {
      dispatch(setProductLoading(true));
      const advancedFilters = getState().product.advancedFilters;
      ...
      dispatch({ type: SET_ADVANCED_FILTERS, payload });
      const sortOrder = getSortOrder(payload.order);
      const response = await axios.get(`${API_URL}/product/list/rated`, {
        params: { ...payload, sortOrder }
      });
      const { products, totalPages, currentPage, count } = response.data;
