...
export const fetchMerchantOrders = () => async dispatch => {
  try {
    dispatch(setOrderLoading(true));
    const response = await axios.get(`${API_URL}/order/merchant-orders`);
    dispatch({ type: FETCH_MERCHANT_ORDERS, payload: response.data.merchantOrders });
  } catch (error) {
...
export const fetchOrders = (page = 1) => async dispatch => {
  try {
    dispatch(setOrderLoading(true));
    const response = await axios.get(`${API_URL}/order`, { params: { page, limit: 20 } });
...
export const fetchAccountOrders = (page = 1) => async dispatch => {
  try {
    dispatch(setOrderLoading(true));
    const response = await axios.get(`${API_URL}/order/me`, { params: { page, limit: 20 } });
    const { orders, totalPages, currentPage, count } = response.data;
    dispatch({ type: FETCH_ORDERS, payload: orders });
    dispatch({ type: SET_ADVANCED_FILTERS, payload: { totalPages, currentPage, count } });
  } catch (error) {
